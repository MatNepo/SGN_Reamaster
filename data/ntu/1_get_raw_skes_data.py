...
def get_raw_bodies_data(skes_path, ske_name, frames_drop_skes, frames_drop_logger):
    ...
    # TODO Should be num_frames_drop < num_frames but it causes an exception (that's why tried <=)
    assert num_frames_drop <= num_frames, \
        (f'Error: All frames data (%d) of %s is missing or lost '
         f'(num_frames_drop: {num_frames_drop} < num_frames: {num_frames})') % (num_frames, ske_name)
...
def get_raw_skes_data():
    ...
    raw_skes_data = []
    frames_cnt = np.zeros(num_files, dtype=np.int32)  # или np.int64, если вам нужен 64-битный целочисленный тип
